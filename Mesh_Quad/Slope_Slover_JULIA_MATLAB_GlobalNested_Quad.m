function [res1 ] = Slope_Slover_JULIA_MATLAB_GlobalNested_Quad(level)
ProcID=2
clc
% close all
NumberOfGaussPoints=[3 5 5 9 9]; %Paper
...
c=dlmread(fileC);
 c=ones(nElem,NumberOfGaussPoints(Order-1)^2)*8000;
if(HigherOrder==0)
     c=ones(nElem,1)*8000;
...
% c=F(nodes_centers(:,1),nodes_centers(:,2));
    ...
    c=transpose(reshape(c,NumberOfGaussPoints(level_num)^2,nElem));
end
