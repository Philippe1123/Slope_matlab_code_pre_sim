...
basename = 'NI_Rotor37_2007_HyperMC';
...
qois = {mass_flow, pressure_ratio, temperature_ratio, efficiency};
qoi_names = {'mass flow', 'pressure ratio', 'temperature ratio', 'efficiency'};
...
for idq = 2:length(quants) - 1
    q = quantile(pressure, [0+quants(idq), 1-quants(idq)], 1);
    shadedErrorBar(x, median(pressure, 1), abs(q' - median(pressure, 1)'), 'lineprops', color, 'patchsaturation', 0.1)
    Q(:, idq) = q(1,:)';
    Q(:, size(Q, 2) - idq + 1) = q(2,:)';
end
plot(x, median(pressure, 1), 'linewidth', 1.5, 'color', color);
