...
if(higherOrder==true)
NumberOfGaussPoints=[7 7 16 28 37];
...
ElementsAll{id+1}=Elements;
NodesAll{id+1}=Nodes;

    Hash=[];
    Center=[];
for hj=1:length(Elements)
 NPos=Nodes(Elements(hj,5:end),2:3);  
%   Hash(hj)=NPos(1,1)+NPos(1,2)^2+NPos(2,1)^2+NPos(2,2)^3+NPos(3,1)^3+NPos(3,2)^4;
  
    Posx=(NPos(1,1)+NPos(2,1)+NPos(3,1))/3;
    Posy=(NPos(1,2)+NPos(2,2)+NPos(3,2))/3;
Center(hj,1)=hj;
Center(hj,2)=Posx;
Center(hj,3)=Posy;

Hash(hj,1)=round(Posx^(1/2)+Posy^(1/4),10);
%   Hash=Hash';
  
end
CenterAll{id+1}=Center;
%  Hash=floor(Hash);
check=length(unique(Hash));
  if(check~=length(Hash))
     fprintf("ERROR ERROR\n") 
  end
Hash=[[1:1:length(Hash)]',Hash];
HashAll{id+1}=Hash;
...
close all
figure
 plotelem(NodesAll{1},ElementsAll{1},Types,'GCS','off','LineWidth',3);
 
 
 
  Elements=ElementsAll{1};
      Nodes=NodesAll{1};
            Center=[];

  for hj=1:length(ElementsAll{1})
...
Center(hj,3)=Posy;
  ...
  Pathcenters=foldername+"/h_ref/Matlab/"+"ElementsCenter_L_"+num2str(0)+".txt";
dlmwrite(Pathcenters,Center(:,2:3),'delimiter',' ','precision',15)   
...
for p=1:length(HashAll)-1
[c,ia]=setdiff(HashAll{1,p+1}(:,2),HashAll{1,p}(:,2),'stable');
NewElem=[HashAll{1,p}(:,2) ;c];
Sortvec=zeros(length(NewElem),1);
H=HashAll{1,p+1};
for(n=1:length(NewElem))
    
 Sortvec(n,1)=find(H(:,2)==NewElem(n));
end
elem=ElementsAll{p+1}(Sortvec,:);
elem(:,1)=[1:1:length(elem)]'
 HashAll{p+1}(:,2)=NewElem;
 ElementsAll{p+1}=elem;
 figure
  plotelem(NodesAll{p+1},ElementsAll{p+1},Types,'GCS','off','LineWidth',3);
        Elements=ElementsAll{p+1};
      Nodes=NodesAll{p+1};
      Center=[];
  for hj=1:length(ElementsAll{p+1})
...
Center(hj,3)=Posy;
  ...
  Pathcenters=foldername+"/h_ref/Matlab/"+"ElementsCenter_L_"+num2str(p)+".txt";
dlmwrite(Pathcenters,Center(:,2:3),'delimiter',' ','precision',15)   


PathElements=foldername+"/h_ref/Matlab/"+"Elements_L_"+num2str(p)+".txt";
dlmwrite(PathElements,ElementsAll{p+1},'delimiter',' ','precision',15)   
